...
subroutine UpdateTendencies(vlen,nlev,do_cldice,deltat,fx,fnx,pdelInv,pdel,qxtend,nxtend, &
                              qxsedten,dumx,dumnx,prect,xflx,xxlx,qxsevap,xcldm,tlat,qvlat,preci)

   integer, intent(in)               :: vlen,nlev
   logical, intent(in)               :: do_cldice
   real(rkind_comp),intent(in)               :: deltat
   real(rkind_comp), intent(in)              :: fx(vlen,nlev)
   real(rkind_comp), intent(in)              :: fnx(vlen,nlev)
   real(rkind_comp), intent(in)              :: pdelInv(vlen,nlev)
   real(rkind_comp), intent(in)              :: pdel(vlen,nlev)
   real(rkind_comp), intent(inout)           :: qxtend(vlen,nlev)
   real(rkind_comp), intent(inout)           :: nxtend(vlen,nlev)
   real(rkind_comp), intent(inout)           :: qxsedten(vlen,nlev)
   real(rkind_comp), intent(inout)           :: dumx(vlen,nlev)
   real(rkind_comp), intent(inout)           :: dumnx(vlen,nlev)
   real(rkind_comp), intent(inout)           :: prect(vlen)
   real(rkind_comp), intent(inout)           :: xflx(vlen,nlev+1)
   real(rkind_comp), intent(in)   , optional :: xxlx
   real(rkind_comp), intent(inout), optional :: qxsevap(vlen,nlev)
   real(rkind_comp), intent(in)   , optional :: xcldm(vlen,nlev)
   real(rkind_comp), intent(inout), optional :: tlat(vlen,nlev)
   real(rkind_comp), intent(inout), optional :: qvlat(vlen,nlev)
   real(rkind_comp), intent(inout), optional :: preci(vlen)
   integer :: i,k,n,nstep
...
subroutine UpdateTendencies_vec(vlen,nlev,do_cldice,deltat,fx,fnx,pdelInv,qxtend,nxtend, &
                              qxsedten,dumx,dumnx,prect,xflx,xxlx,qxsevap,xcldm,tlat,qvlat,preci)
   ...
   real(rkind_comp), intent(inout)           :: dumnx(vlen,nlev)
   real(rkind_comp), intent(inout)           :: prect(vlen)
   real(rkind_comp), intent(inout)           :: xflx(vlen,nlev)
   real(rkind_comp), intent(in)   , optional :: xxlx
   real(rkind_comp), intent(inout), optional :: qxsevap(vlen,nlev)
   real(rkind_comp), intent(in)   , optional :: xcldm(vlen,nlev)
   real(rkind_comp), intent(inout), optional :: tlat(vlen,nlev)
   ...
   real(rkind_comp) :: faltndx(vlen),faltndnx(vlen)
   real(rkind_comp) :: rnstep(vlen),faltndqxe(vlen)
   real(rkind_comp) :: faltndqxe2(vlen)
   real(rkind_comp) :: faloutx(vlen,nlev),faloutnx(vlen,nlev),dum1(vlen)
   real(rkind_comp) :: tmpk1(nlev),tmpk2(nlev)
   real(rkind_comp) :: tmp1(vlen,nlev),tmp2(vlen,nlev)
   real(rkind_comp) :: mask(vlen)
   integer  :: iters(vlen)
